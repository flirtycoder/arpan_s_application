...
const Footer = () => {
  ...
  return (
    <footer className="bg-black/50 backdrop-blur-sm text-white py-4">
        ...
        <nav className="flex flex-wrap justify-center">
          {categories.map((category, index) => (
            <Link 
              key={index}
              href={category.href}
              className={`px-3 py-2 text-sm uppercase tracking-wide ${category.highlight ? 'text-red-400' : 'text-white'} hover:opacity-80`}
            >
              {category.name}
            </Link>
          ))}
