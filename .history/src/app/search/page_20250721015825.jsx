'use client';
import AnnouncementBar from '@/components/ui/announcementBar';
import Header from '@/components/common/Header';
import Ending from '@/components/common/Ending';
import Image from 'next/image';

const productImages = [
  '/search2.svg',
  '/search3.svg',
  '/search4.svg',
  '/search5.svg',
  '/search6.svg',
  '/search7.svg',
  '/search8.svg',
  '/search9.svg',
  '/search10.svg',
  '/search11.svg',
  '/search12.svg',
  '/search6.svg',
];

const Page = () => {
  return (
    <div className="relative bg-white min-h-screen overflow-hidden">
      {/* ✅ Background Image - Force Visibility */}
      <div className="absolute left-0 bottom-20 z-0 w-[927px] h-[960px]">
        <Image src="/search1.svg" alt="Background Art" fill className="object-contain" priority />
      </div>

      {/* Main Content Above Background */}
      <div className="relative z-10">
        <AnnouncementBar
          message="Get early access on launches and offers."
          linkText="Sign Up For Texts"
          linkUrl="/signup"
        />
        <Header />

        {/* Search Bar */}
        <div className="flex justify-center my-10">
          <div className="flex items-center w-full max-w-2xl bg-[#F6F6F6] border border-gray-300 rounded-md px-4 py-2">
            <input
              type="text"
              placeholder="Search"
              className="flex-grow bg-transparent outline-none text-sm text-gray-700 placeholder-gray-500"
            />
            <button>
              <svg width="20" height="20" fill="none" stroke="gray" strokeWidth="1.5">
                <circle cx="9" cy="9" r="6" />
                <line x1="14" y1="14" x2="19" y2="19" />
              </svg>
            </button>
          </div>
        </div>

        {/* Masonry Grid of Images */}
        <div className="columns-2 sm:columns-3 md:columns-4 gap-4 space-y-4 px-4 sm:px-10 md:px-20">
          {productImages.map((src, index) => (
            <div key={index} className="break-inside-avoid overflow-hidden rounded-lg shadow-sm">
              <img
                src={src}
                alt={`Product ${index + 1}`}
                className="w-full h-auto rounded-lg object-cover"
              />
            </div>
          ))}
        </div>

        <Ending />
      </div>
    </div>
  );
};

export default Page;
